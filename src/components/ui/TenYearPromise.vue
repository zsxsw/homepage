  ...
  <div class="relative bg-gradient-to-r from-slate-600/80 to-stone-700/80 dark:from-slate-700/80 dark:to-stone-800/80 rounded-lg p-6 text-white shadow-xl backdrop-blur-lg border border-white/20 dark:border-gray-700/20">
      ...
      <div>
        ...
        <div class="flex items-center space-x-4">
          <div class="text-center">
            <div class="text-3xl font-bold">{{ persistedDays }}</div>
            <div class="text-xs text-slate-200 dark:text-slate-300">天</div>
          </div>
          <div class="text-center">
            <div class="text-lg font-semibold">{{ persistedYears }}</div>
    ...
    <div class="mt-5 bg-white/20 rounded-lg p-4 border border-white/30 backdrop-blur-sm shadow-inner">
       ...
       <div class="bg-white/20 rounded-full h-2 mb-3">
         <div 
           class="bg-amber-300 dark:bg-amber-400 rounded-full h-2 transition-all duration-500 ease-out"
           :style="{ width: progressPercentage + '%' }"
         ></div>
       </div>
       <div class="flex justify-between text-xs text-white/80">
         <span>已坚持 {{ persistedDays }} 天</span>
...
const persistedDays = computed(() => {
  return daysPassed.value
})

const persistedYears = computed(() => {
  return (persistedDays.value / 365).toFixed(1)
})
...
const remainingDays = computed(() => {
  const tenYearTarget = 10 * 365
  return Math.max(tenYearTarget - persistedDays.value, 0)
})
